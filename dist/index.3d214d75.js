...
else renderFavorites((0, _storageJs.favoriteCities));
if (localStorage.getItem("currentCity")) getWeather((0, _storageJs.currentCity));
// Переключение табов
document.querySelectorAll(".tab").forEach((item)=>item.addEventListener("click", function(event1) {
        event1.preventDefault();
        const id = event1.target.getAttribute("href").replace("#", "");
        document.querySelectorAll(".tab").forEach((child)=>child.classList.remove("active-tab"));
        document.querySelectorAll(".tab__item").forEach((child)=>child.classList.remove("active"));
        item.classList.add("active-tab");
        document.getElementById(id).classList.add("active");
    }));
document.querySelector(".tab").click();
// Получение погоды
(0, _viewJs.UI_ELEMENTS).FIND_FORM.addEventListener("submit", handlerCitySearch);
(0, _viewJs.UI_ELEMENTS).FIND_CITY.addEventListener("click", handlerCitySearch);
function handlerCitySearch(event1) {
    event1.preventDefault();
    const city = (0, _viewJs.UI_ELEMENTS).FIND_INPUT.value;
    getWeather(city);
}
async function getWeather(city) {
    ...
    try {
        ...
        } else alert("Ошибочка вышла: " + response.status);
    } catch (error) {
        alert(error.stack);
    }
    (0, _viewJs.UI_ELEMENTS).FIND_INPUT.value = "";
}
...
function renderNow(temp, city, icon) {
    clearTab((0, _viewJs.UI_ELEMENTS).TAB_NOW);
    const p1 = document.createElement("p");
    p1.classList.add("tab-now__temperature");
    p1.textContent = temp + "\xb0";
    (0, _viewJs.UI_ELEMENTS).TAB_NOW.appendChild(p1);
    const p2 = document.createElement("p");
    p2.classList.add("tab-now__city");
    p2.textContent = city;
    (0, _viewJs.UI_ELEMENTS).TAB_NOW.appendChild(p2);
    const input = document.createElement("input");
    input.classList.add("tab-now__add");
    input.type = "button";
    input.addEventListener("click", ()=>addToFavorites(city));
    (0, _viewJs.UI_ELEMENTS).TAB_NOW.appendChild(input);
    const img = document.createElement("img");
    img.classList.add("tab-now__img");
    img.src = icon;
    img.alt = "weather icon";
    (0, _viewJs.UI_ELEMENTS).TAB_NOW.appendChild(img);
}
...
function renderFavorites(cities) {
    while((0, _viewJs.UI_ELEMENTS).ADDED_CITIES_LIST.firstChild)(0, _viewJs.UI_ELEMENTS).ADDED_CITIES_LIST.removeChild((0, _viewJs.UI_ELEMENTS).ADDED_CITIES_LIST.firstChild);
    for (const elem of cities){
        ...
        (0, _viewJs.UI_ELEMENTS).ADDED_CITIES_LIST.append(li);
    }
}
// Удаление из избранного
function delFromFavorites(event1, cities, elem) {
...
function renderDetails(data) {
    while((0, _viewJs.UI_ELEMENTS).TAB_DETAILS.firstChild)(0, _viewJs.UI_ELEMENTS).TAB_DETAILS.removeChild((0, _viewJs.UI_ELEMENTS).TAB_DETAILS.firstChild);
    ...
    (0, _viewJs.UI_ELEMENTS).TAB_DETAILS.appendChild(p);
    ...
    (0, _viewJs.UI_ELEMENTS).TAB_DETAILS.appendChild(ul);
}
function renderForecast(data) {
    while((0, _viewJs.UI_ELEMENTS).TAB_FORECAST.firstChild)(0, _viewJs.UI_ELEMENTS).TAB_FORECAST.removeChild((0, _viewJs.UI_ELEMENTS).TAB_FORECAST.firstChild);
    const monthes = [
        "дек",
        "янв",
        "фев",
    ...
    (0, _viewJs.UI_ELEMENTS).TAB_FORECAST.appendChild(p);
    for(let i = 0; i <= 10; i++){
        let month = (0, _dateFns.format)(new Date(dataForecast[i].dt * 1000), "MMM");
        let day = (0, _dateFns.format)(new Date(dataForecast[i].dt * 1000), "dd");
        ...
        div.appendChild(img);
        (0, _viewJs.UI_ELEMENTS).TAB_FORECAST.appendChild(div);
    }
}

},{"./view.js":"de63B","./storage.js":"kS9sU","date-fns":"9yHCA"}],"de63B":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "UI_ELEMENTS", ()=>UI_ELEMENTS);
const UI_ELEMENTS = {
    TABS: document.querySelectorAll(".tab"),
    TAB_ITEMS: document.querySelectorAll(".tab-item"),
    FIND_FORM: document.querySelector(".find-form"),
...
},{}],"kS9sU":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["awEvQ","bB7Pu"], "bB7Pu", "parcelRequiref2f1")
